...
int main() {
	...
	for (int i = 0; i < 8; i++) {
		cout << endl << "DFS from " << i << ": ";
		graph.DFS(i);

		cout << endl << "BFS from " << i << ": ";
		graph.BFS(i);
	...
	for (int i = 0; i < 8; i++) {
		for (int j = 0; j < 8; j++) {
			if (graph.IsVertexReachable(i, j)) {
				cout << endl << "Vertex " << i << " is reachable from vertex " << j;
			}
			else {
				cout << endl << "Vertex " << i << " is not reachable from vertex " << j;
			}
		}
